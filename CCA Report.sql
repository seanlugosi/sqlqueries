runonstagclient 51536 "SELECT DISTINCT parsed.last_click_time AS 'Click Time',
  parsed.conversion_time AS 'Conversion Time',
  parsed.order_type AS 'Conversion Type',
  ct.conversion_type_id,
  parsed.uuid as 'UUID',
  parsed.order_id AS 'Order ID',
  ...
  parsed.quantity AS 'Conversions',
  parsed.price AS 'Revenue',
  parsed.currency AS 'Currency'
FROM 
(
  SELECT deduped.day,
    deduped.conversion_time,
    deduped.last_click_time,
    ...
    deduped.quantity,
    deduped.price,
    deduped.keyword_instance_id,
    deduped.creative_id,
    deduped.publisher_group_id,
  ...
  (
    SELECT   
      DATE(CONVERT_TZ(conv.receive_time, 'GMT', clients.time_zone)) AS DAY,
      conv.uuid,
      conv.order_type,
      ...
      conv.quantity,
      conv.price,
      conv.keyword_instance_id,
      conv.creative_id,
      conv.publisher_group_id,
      MIN(CONVERT_TZ(conv.receive_time, 'GMT', clients.time_zone)) AS conversion_time,
      MAX(CONVERT_TZ(clicks.receive_time, 'GMT', clients.time_zone)) AS last_click_time
    FROM marin_olap_staging.tracker_data_51536 conv 
      JOIN marin_olap_staging.tracker_data_51536 clicks ON conv.uuid = clicks.uuid 
    ...
    WHERE conv.action_id = 2
      ...
      AND conv.receive_time BETWEEN CONVERT_TZ('2016-10-24', clients.time_zone, 'GMT')
        AND CONVERT_TZ('2016-10-25',  clients.time_zone, 'GMT') + INTERVAL 1 DAY
      AND clicks.receive_time >= SUBDATE(conv.receive_time, INTERVAL clients.conversion_window DAY) 
      AND clicks.receive_time <= conv.receive_time
    GROUP BY
      DATE(CONVERT_TZ(conv.receive_time, 'GMT', clients.time_zone)),
      conv.uuid,
      conv.order_type,
    ...
    JOIN marin_olap_staging.tracker_data_51536 click ON deduped.uuid = click.uuid AND deduped.last_click_time = CONVERT_TZ(click.receive_time, 'GMT', clients.time_zone) AND click.action_id = 1
    JOIN marin.client_keyword_params mkwid on clients.client_id = mkwid.client_id and mkwid.search_order = 0 
...
AND c.client_id = 51980
ORDER BY conversion_time;" > 51980_test_report.txt